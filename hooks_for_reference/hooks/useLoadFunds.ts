...
import { Boolean } from "aws-sdk/clients/cloudwatchlogs";
...
export type GenericTokenData = {
  ...
  investor: { principal: number; junior: number, senior: number,pending: number, j_pending: number, s_pending: number; isFrozen: boolean };
...
export function useBorrowGeneric(
    ...
    const borrowGeneric = (V: LoanVoucherGeneric, fastDraw : Boolean) =>
      runTx(
        async () => {
          ...
          // INPUTFUND contract expects voucher.loanId, but it has voucher.loanID. UPDATE IN CONTRACT!
          const {borrower,chosenAmount,chosenRate,_fastDraw,loanId,loanType,maturityTs,maxAmount,minRateBP,paramsHash,raw_maxAmount,sig,union} = V
          ...
          // testing signature correctness by calling recoverVoucherSigner
          /*
          const signer = viewer.recoverVoucherSigner(
            '0xBea16D53399d5b6627D6c30aFDfB3f6482D5932B',
            wallet?.address,
            union,
            BigInt(maxAmount),    // uint256
            minRateBP,            // uint16
            loanType32,           // must be 0x-prefixed 32-byte hex
            paramsHash32,         // 0x… length 66
            fastDraw    ,          // bool
            to0x(sig)                  // 0x… signature
          )
          console.log('signer', signer)
          console.log(
            'union', union,
            'loanIdBytes32', loanIdBytes32,
            'loanType32', loanType32,
            'chosenAmount', BigInt(chosenAmount),
            'rate', Number(rate),
            'maturityTs', Number(maturityTs),
            'paramsHash32', paramsHash32, 
            'sig', sig, 
          ...
          */
          ...
          return core.drawLoanWithVoucher(
            ...
            loanIdBytes32,
            loanType32,           // must be 0x-prefixed 32-byte hex
            BigInt(chosenAmount), // uint128 → bigint is fine in v6
            rate,                 // uint16
        ...
        },
        {
          ...
          onSuccess: () => {
            ...
            const debtItem = { 
              ...
              principalRepaid: 0,
              rateBP: V.minRateBP,
              type: fastDraw ? 'GENERIC' : 'PENDING',
              union: V.union,
              digestTs: fastDraw ? 1n : 0n // matched getBorrowerInfo on useLoan
            }
