...
export function useFilterTasks(LAND, CAP) {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const fallowFields = !!db?.reloadActivity?.act?.features?.some(f => f?.properties?.class === 0 || f?.properties?.class === 1);
      ...
      let polBalance = null;
      let baseNin = 40 
      if (provider && db?.address) {
        try {
          const raw = await provider.getBalance(db.address);
          polBalance = Number(ethers.formatEther(raw)); 
        } catch {}
      }
      const POL_or_Flag = (typeof polBalance === 'number' && polBalance < 0.2) && polBalance || topUpGasFlag;
